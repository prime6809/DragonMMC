...
CloseFilesInit2          puls    u                        ; put the return address in u                                        
...
; read one sector
DosFunctionReadSec
                lda     #WDReadSector           ; $80 is read flag (1793 read sector)                                                  
ld7fa
                fcb     Skip2                   ; cmpx
		...
		STB     DosErrorMask
                ...
                lda	dsec			; get sector no
		jsr     >MMC_SendHR             ; Send head (always 0) and sector no
                
                lda	dcdrv			; get drive
		ldx	dcbpt                   ; point x to i/o buffer  
		lbsr	MMC_ReadDOSSec		; go read it
	
		anda	#WDErrMaskRW		; Mask out error bits
		sta     dcsta                   ; save in dskcon status 	
		puls	x,u,pc
		
                ifeq    1
                
GetLBA		lda	dctrk			; get track
		ldb	#SectorsPerTrack	; get sectors / track
		mul				; workout track offset
		tfr	d,x			; move to x
		ldb	dsec			; get sector no
...
;               lda     #WDWriteSector          ; $a0 is write flag (1793 write sector)                                                    
               lda      #1
		...
		tfr	a,b			; get track in b
		lda	dcdrv			; Get drive no
		lbsr	MMC_SeekCheck		; Go try and seek the image
		
;               lda     #WDSeek                  ; seek command for 1793: do not load the                                                
;               sta     fdcreg                   ; head at start, verify destination track,                                                    
;               exg     a,a                      ; 30 ms stepping rate - wait for                                       
;               exg     a,a                      ; valid status from 1793                               
;               bsr     WaitFDCNotBusy           ; wait till not busy                             
...
;               bsr     MediumDelay             ; wait some more                         
               ...
               sta     dcsta                    ; save in dskcon status                                
ld82a          puls    a,pc  			; error : return to caller     
...
ld82c          lda     dsec                     ; get sector number desired                                        
               ...
               ldy     Misc16BitScratch         ; zero out y - timeout initial value                                            
...
;               cmpb    #WDReadSector            ; was this a read?                           
