...
void UpdateCPLD(void)
{
	int	result;
	
	log0("Updating CPLD\n");
	log0("Opening CPLD firmware file\n");
	
	strcpy_P((char *)globalData,PSTR(FirmwareFile));
	log0("globalData=%s\n",globalData);
	result = fileOpen(FA_OPEN_EXISTING|FA_READ,CAS_FILE);
	
	if(result)
	{
		log0("ERROR 0x%02X opening CPLD firmware file " FirmwareFile "\n");
	}
	else
	{
		InitJTAG();
		result = xsvfExecute();
		fileClose(CAS_FILE);
		log0("xsvfExecute() returned %d\n",result);
		log0("CPLD update completed\n");
		ReleaseJTAG();
	}
}
